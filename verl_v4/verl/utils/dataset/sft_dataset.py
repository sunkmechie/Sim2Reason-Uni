...
class SFTDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        # Helper function to extract text and calculate tokenized length
        def _get_length_of_row(row):
            prompt_key = self.prompt_key[0]
            response_key = self.response_key[0]
            # Extract prompt text
            prompt = row[prompt_key]

            # prompt = series_to_item(prompt)

            # Extract response text
            response = row[response_key]

            # response = series_to_item(response)
                
            # Apply chat template and tokenize to find length
            ...
            response_ids = self.tokenizer(response_chat_str, add_special_tokens=False)["input_ids"]
            
            return len(prompt_ids) + len(response_ids)

        # Calculate total token length for each row and filter
        total_lengths = self.dataframe.apply(_get_length_of_row, axis=1)
        self.dataframe = self.dataframe[total_lengths <= self.max_length].reset_index(drop=True)

        # Now, create the final prompts and responses lists from the filtered dataframe
        ...
        print("Dataset size after filtering by max_length:", len(self.prompts))

    def _read_files_and_tokenize_old(self):
        def series_to_item(ls):
            import numpy
            import pandas
