...
class ValidationGenerationsLogger:
    ...
    def log(self, loggers, samples, step, extra_columns=None):
        if "wandb" in loggers:
            return self.log_generations_to_wandb(samples, step, extra_columns = extra_columns)
    ...
    def log_generations_to_wandb(self, samples, step, extra_columns=None):
        import wandb

        self._log_generations_to_wandb(samples, step, wandb, extra_columns=extra_columns)

    def _log_generations_to_wandb(self, samples, step, wandb, extra_columns=None):
...
@dataclasses.dataclass
class ValidationGenerationsMultiRewardLogger(ValidationGenerationsLogger):
    def log_generations_to_wandb(self, samples, step, extra_columns=None):
        """Log samples to wandb as a table"""
        import wandb
        ...
        # Create a new table with same columns and existing data
        # Workaround for https://github.com/wandb/wandb/issues/2981#issuecomment-1997445737
        samples = [[step]+list(sample) for sample in samples]
        new_table = wandb.Table(columns=columns, data=samples)
