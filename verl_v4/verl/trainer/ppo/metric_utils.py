        ...
        for var_name, metric2prompt_vals in var2metric2prompt_vals.items():
            for metric_name, prompt_vals in metric2prompt_vals.items():
                if metric_name.startswith("mean@"):
                    # For mean metrics, calculate both mean and MODIFIED std_of_means metric
                    n_samples = metric_name.split("@")[1]
                    data_src2var2metric2val[data_source][var_name][metric_name] = np.mean(prompt_vals)
                    if len(prompt_vals) > 1:
                        # MODIFIED: Calculate std of answer-position-level means (batch-level accuracy for each answer position)
                        # Get raw data for this data source and variable to calculate answer-position-level means
                        answer_position_means = []
                        
                        # Collect all values by answer position
                        position_vals = defaultdict(list)
                        for prompt, var2vals in data_src2prompt2var2vals[data_source].items():
                            if var_name in var2vals:
                                for position, val in enumerate(var2vals[var_name]):
                                    position_vals[position].append(val)
                        
                        # Calculate batch-level mean for each answer position
                        for position in sorted(position_vals.keys()):
                            if len(position_vals[position]) > 0:
                                position_mean = np.mean(position_vals[position])
                                answer_position_means.append(position_mean)
                        # Calculate std of answer-position-level means (use ddof=0 for consistency)
                        if len(answer_position_means) > 1:
                            data_src2var2metric2val[data_source][var_name][f"std_of_means@{n_samples}"] = np.std(answer_position_means, ddof=0)
                        else:
                            data_src2var2metric2val[data_source][var_name][f"std_of_means@{n_samples}"] = 0.0
                else:
                    # For all other metrics (including original std), keep the original logic
                    data_src2var2metric2val[data_source][var_name][metric_name] = np.mean(prompt_vals)

    # NEW: Calculate global std across all responses (not per-prompt std averaged)
    # This provides a more accurate measure of overall variance
    data_src2var2global_vals = defaultdict(lambda: defaultdict(list))
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], str):
                    continue
                # Collect all values for this data_source and var_name
                data_src2var2global_vals[data_source][var_name].extend(var_vals)

    # Calculate global std for each data_source and var_name
    for data_source, var2global_vals in data_src2var2global_vals.items():
        for var_name, global_vals in var2global_vals.items():
            if len(global_vals) > 1:
                n_total = len(global_vals)
                global_std = np.std(global_vals)
                # Add global std metric with a distinguishing name
                data_src2var2metric2val[data_source][var_name][f"global_std@{n_total}"] = global_std
