...
class FSDPSFTTrainer:
    def __init__(
        ...
        if rank == 0:
            ...
            experiment_name = tracking.logger["wandb"].run.name
            object_list[0] = experiment_name
        
        broadcast_object_list(object_list, src=0)
        experiment_name = object_list[0]

        if rank != 0:
            print(f"[rank {rank}] Using experiment name:", experiment_name)
        
        self.config.trainer.default_local_dir = os.path.join(
            self.config.trainer.default_local_dir, experiment_name
        )
    ...
    def _normalize_config_bsz(self):
        ...
        assert self.config.data.train_batch_size % self.config.data.micro_batch_size_per_gpu == 0, f"train_batch_size {self.config.data.train_batch_size} is not divisible by micro_batch_size_per_gpu {self.config.data.micro_batch_size_per_gpu}"
