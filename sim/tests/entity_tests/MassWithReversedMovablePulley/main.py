...
def main(cfg: DictConfig):
    ...
    ##### INFO #####
    # MassWithReversedMovablePulley scene:
    #   - MassWithReversedMovablePulley entity: 2 hanging masses (4.36, 4.29 kg), pulley 0.15 kg
    #   - MassWithFixedPulley entity: hanging mass 2.6 kg
    #   - Connected via tendon — Atwood machine with reversed movable pulley
    ...
    print(f"\n{'='*60}")
    print(f"PHYSICS ASSERTIONS — MassWithReversedMovablePulley")
    ...
    hanging = [(n, bd) for n, bd in body_data.items() if "pulley" not in n.lower()]
    if len(hanging) >= 2:
        max_dz = 0
        for name, bd in hanging:
            dz = abs(bd["position"][-1, 2] - bd["position"][0, 2])
            max_dz = max(max_dz, dz)
            print(f"  {name}: Δz = {bd['position'][-1, 2] - bd['position'][0, 2]:.4f}")
        assert max_dz > 0.01, "No significant motion detected"
