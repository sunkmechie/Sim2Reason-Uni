...
def main(cfg: DictConfig):
    ...
    ##### INFO #####
    # MassWithFixedPulley test scene: 
    #   - MassWithReversedMovablePulley entity with two hanging masses (4.36, 4.29 kg)
    #     and a pulley mass (0.15 kg)
    #   - Connected to a MassWithFixedPulley entity with hanging mass (2.6 kg)
    #   - Standard Atwood machine variant with movable pulley
    ...
    print(f"\n{'='*60}")
    print(f"PHYSICS ASSERTIONS — MassWithFixedPulley")
    ...
    #    For frictionless pulley systems, energy should be conserved.
    #    Note: string PE is zero (inextensible), only KE + gravitational PE.
    # ================================================================
    ...
    # ================================================================
    # 5. HEAVIER MASS DESCENDS
    #    In an Atwood-like system, the heavier side should descend.
    # ================================================================
    print("--- [5] Checking system motion under gravity ---")
    # Find bodies that are hanging (not fixed pulleys)
    hanging_bodies = [(name, bd) for name, bd in body_data.items()
                      if "pulley" not in name.lower()]
    if len(hanging_bodies) >= 2:
        # At least some body should have moved vertically
        max_z_change = 0
        for name, bd in hanging_bodies:
            z_change = abs(bd["position"][-1, 2] - bd["position"][0, 2])
            max_z_change = max(max_z_change, z_change)
            print(f"  {name}: Δz = {bd['position'][-1, 2] - bd['position'][0, 2]:.4f}")
        assert max_z_change > 0.01, "No significant motion detected"
    print("  ✓ System motion verified.\n")
