...
def main(cfg: DictConfig):
    ...
    ##### INFO #####
    # MassPrismPulleyPlane: Blocks on a wedge (prism) sitting on a flat plane
    #   - Two blocks on left/right sides of a wedge, connected by a string over the prism apex
    #   - prism_left_slope=45°, prism_right_slope=60°
    #   - No external friction, no external connections/pulleys
    #   - Under gravity, the heavier (or steeper-side) block pulls the lighter one
    ...
    print(f"\n{'='*60}")
    print(f"PHYSICS ASSERTIONS — MassPrismPulleyPlane")
    print(f"  {N} timesteps, g={g_mag}")
    print(f"{'='*60}\n")

    # Discover all movable bodies
    body_data = {}
    for key in data.keys():
        if key == "global":
            continue
        if "position" not in data[key]:
            continue
        bd = data[key]
        body_data[key] = {
        ...
        if "velocity_angular" in bd:
            body_data[key]["velocity_angular"] = np.array(bd["velocity_angular"])
    ...
    for name, bd in body_data.items():
        if "acceleration" not in bd or "net_force" not in bd:
            continue
        # Recorder "net_force_linear" comes from a full spatial-inertia model.
        # For rotating extended rigid bodies (like prisms), scalar translational F=ma is not directly comparable.
        if ".prism" in name:
            print(f"  {name}: skipped (rigid-body prism; translational F=ma is not directly applicable)")
            continue
        m = bd["mass"]
    ...
    #    Total E = sum(KE) + sum(m*g*z). With friction=0, energy should
    #    be conserved. With friction, energy should not increase.
    # ================================================================
