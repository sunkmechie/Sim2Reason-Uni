...
def main(cfg: DictConfig):
    ...
    ##### INFO #####
    # MassPrismPlaneEntity: Mass block on a prism (wedge) on a flat plane
    #   - Connected to a MassWithFixedPulley via tendon
    #   - prism_left_slope=30°, prism_right_slope=60°
    #   - block_mass=2.8, prism_mass=3.4, no friction
    ...
    print(f"\n{'='*60}")
    print(f"PHYSICS ASSERTIONS — MassPrismPlaneEntity")
    ...
    for name, bd in body_data.items():
        if "acceleration" not in bd or "net_force" not in bd:
            continue
        # Recorder "net_force_linear" is computed from a full spatial-inertia model.
        # For rotating extended bodies (e.g., prisms), comparing it to scalar m*a is not valid.
        if ".prism" in name:
